           ...
           scRet == SEC_I_INCOMPLETE_CREDENTIALS ) 
   {
           ...
           FAILED(scRet) && (dwSSPIOutFlags & ISC_RET_EXTENDED_ERROR))
        {
            if(OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL)
            {
                ...
                printf("%d bytes of handshake data sent\n", cbData);
                if(fVerbose) { PrintHexDump(cbData, (unsigned char*)OutBuffers[0].pvBuffer); printf("\n"); }
...
//      if( scRet == SEC_E_INCOMPLETE_MESSAGE - Input buffer has partial encrypted record, read more
        ...
        if(pDataBuffer)
        {
                    ...
                    {
                        buff = (unsigned char*)pDataBuffer->pvBuffer; // printf( "n-2= %d, n-1= %d \n", buff[length-2], buff[length-1] );
                                                                ...
                                                                CtxtHandle *    phContext)  // in
{
    ...
    pbIoBuffer       = (unsigned char*)LocalAlloc(LMEM_FIXED, cbIoBufferLength);
